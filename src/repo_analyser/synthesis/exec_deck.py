    ...
    slides.append(_slide(
        ...
        [f"{n_repos} repos analyzed, {total_commits} total commits.",
         ...
         (f"Duplication: {exact_dup.get('cross_repo_identical_same_path_groups', 0) if exact_dup else 0} "
          f"files are byte-identical across multiple repos "
          f"({dup.get('cross_repo_clone_pairs', 0) if dup else 0} additional cross-repo block-level clone "
          f"pairs found by jscpd -- a broader, block-level measure, kept separate from the byte-exact "
          f"count above because the two answer different questions)."
          if exact_dup else "Single-repo target -- cross-repo duplication does not apply.")],
