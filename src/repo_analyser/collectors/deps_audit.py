...
def _audit_one_repo(r: Path) -> tuple[list[CVEFinding], list[OutdatedPackage], dict[str, str]]:
    ...
    return all_cves, all_outdated, errors
...
def run_deps_audit(repos: list[Path], out_dir: Path) -> Path:
    ...
    all_outdated: list[OutdatedPackage] = []
    errors: dict[str, str] = {}
    # each repo's osv-scanner+npm-outdated pair is two independent,
    # I/O-bound subprocess calls -- see core.util.run_concurrent's
    # docstring.
    for repo_cves, repo_outdated, repo_errors in run_concurrent(repos, _audit_one_repo):
