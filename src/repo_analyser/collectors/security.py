...
def _scan_one_repo(r: Path, tmp_dir: Path) -> tuple[list[SecretFinding], list[SemgrepFinding], dict[str, str]]:
    ...
    return secrets, semgrep_findings, errors
...
def run_security(repos: list[Path], out_dir: Path, tmp_dir: Path) -> tuple[Path, Path]:
    ...
    # each repo's gitleaks+semgrep pair is two independent, I/O-bound
    # subprocess calls -- see core.util.run_concurrent's docstring. Each
    # thread returns its own results; merging happens here, sequentially,
    # rather than mutating these three shared lists/dict from multiple
    # threads directly.
    for repo_secrets, repo_semgrep, repo_errors in run_concurrent(repos, lambda r: _scan_one_repo(r, tmp_dir)):
        secrets.extend(repo_secrets)
        semgrep_findings.extend(repo_semgrep)
