...
Validate this classifier's accuracy with `chronicle.ontology_audit` before
trusting its headline percentages -- see docs/METHODOLOGY.md.
"""
...
COMMIT_SEP = "\x02==CHRONICLE-COMMIT==\x02"
...
FILE_RULES: list[tuple[str, re.Pattern]] = [
    ...
    ("test", re.compile(r"(^|/)(__tests__|tests?)/|\.(test|spec)\.[jt]sx?$|"
...
KEYWORD_RULES: list[tuple[str, re.Pattern]] = [
    ...
    ("dependency_bump", re.compile(r"\b(bump|upgrade|update)\b.*\b(deps?|dependency|dependencies|version)\b", re.IGNORECASE)),
    ...
    ("feature", re.compile(r"\b(add(ed|s|ing)?|implement(ed|s|ing)?|introduce[sd]?|support for|new )\b", re.IGNORECASE)),
]
