...
from .lang import detect_repo_language, DEPGRAPH_SUPPORTED
...
EXCLUDE_DIR_PARTS = {"node_modules", "dist", "build", ".next", "coverage", ".turbo"}
...
def _analyze_js_repo(repo: Path, out_dir: Path) -> RepoDepGraph:
    ...
    cmd = ["npx", "--yes", "dependency-cruiser", *[str(Path(f).relative_to(repo)) for f in files],
           "--no-config", "--output-type", "json"]
    if ts_config.exists():
        cmd += ["--ts-config", "tsconfig.json"]
    res = run(cmd, cwd=repo, check=False, timeout=180)
...
def _aggregate(data: dict, repo_name: str) -> "RepoDepGraph":
    ...
    def is_internal(resolved: str) -> bool:
        ...
        # expand it to a literal node_modules/... path (package "exports"
        # map resolution) -- only a relative/rooted path is this repo's own code.
...
PY_EXCLUDE = {"node_modules", "dist", "build", ".git", "venv", ".venv", "__pycache__", "site-packages"}
...
def _analyze_go_repo(repo: Path, out_dir: Path) -> RepoDepGraph:
    ...
    module_line = next((l for l in go_mod.read_text().splitlines() if l.startswith("module ")), None)
    if not module_line:
...
def run_depgraph(repos: list[Path], out_dir: Path) -> Path:
    rows = [asdict(analyze_repo(r, out_dir)) for r in repos]
