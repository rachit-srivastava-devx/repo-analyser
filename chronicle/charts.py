...
Mono) are bundled in chronicle/fonts/ and registered with matplotlib
...
def dual_axis_trend(x_labels: list[str], series_a: list[float], series_b: list[float],
    ...
    leg = fig.legend(lines, [l.get_label() for l in lines], loc="upper left",
                      bbox_to_anchor=(0.06, 0.865), frameon=False, ncol=2,
                      prop=F_MONO, fontsize=9)
    for text, color in zip(leg.get_texts(), [MUTED, ACCENT]):
...
def stacked_share_bar(period_labels: list[str], category_series: dict[str, list[float]],
    ...
    for cat in cats:
        ...
        running = [r + v for r, v in zip(running, category_series[cat])]
...
def lorenz_curve(sorted_shares_cumulative: list[float], title: str, out_path: Path,
    ...
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1)
    ax.set_xlabel(f"cumulative share of {entity_label} (least -> most active)", color=MUTED, fontsize=9, fontproperties=F_MONO)
