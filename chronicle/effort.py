...
This is the one category the reference engagement (chronicle_button) built
that chronicle-analyzer didn't originally have. Two differences from that
engagement, stated rather than hidden:

1. Toil clusters there were found by an analyst reading representative
   diffs by hand for 8 hand-picked concentrations. Here they're found
   mechanically: non-delivery commits are grouped by (superclass, the
   ...
   *candidate* worth a human diff-read, not a confirmed finding the way
   chronicle_button's were.
...
def detect_toil_clusters(commits: list[dict]) -> list[dict]:
    ...
    a real, rerunnable group-by, but unlike chronicle_button's toil
    deep-dives, nobody has read representative diffs for each cluster
    here. Treat each row as "worth a human diff-read," not confirmed."""
