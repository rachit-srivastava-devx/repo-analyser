...
def _fix_commits(repo: Path) -> list:
    ...
    for commit in Repository(str(repo), only_no_merge=True).traverse_commits():
        leaf, _rule = classify_commit(commit.msg.splitlines()[0] if commit.msg else "",
                                       [m.new_path or m.old_path for m in commit.modified_files if (m.new_path or m.old_path)])
